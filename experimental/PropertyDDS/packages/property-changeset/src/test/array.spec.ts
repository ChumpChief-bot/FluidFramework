...
describe("Array Operations", () => {
	...
	const generateNamedEntities = (count, offsets?, type?) =>
		range(count).map((i) => {
			const offsetShift = offsets === undefined ? undefined : offsets.shift();
			const id = offsetShift === undefined ? guidCounter++ : guidCounter - offsetShift;
			if (type === undefined) {
				return {
					String: {
						guid: `00000000-0000-0000-0000-${`000000000000${id}`.slice(-12)}`,
					},
	...
	function validateChangeSet(CS) {
		...
		if (arrayCS.insert) {
			for (const x of arrayCS.insert) insertPositions.add(x[0]);
		}
		for (const type of ["insert", "modify", "remove"]) {
			...
			if (changes !== undefined) {
				...
				for (const entry of changes) {
					...
					lastIndex = entry[0];
					lastLength = isNumber(entry[1]) ? entry[1] : entry[1].length;

					// Inserts must not lie within modify or remove ranges
					if (type === "remove" || type === "modify") {
						for (const i of insertPositions)
							assert(
					...
					it(`at position ${i + offset}${additionalInserts}`, () => {
						...
						testApplyAssociativity(initial, ops, (combinedCS) => {
							...
							expect(arrayCS.insert.length).to.equal(
								(i >= 2 && i < 8 ? 2 : 3) + (additionalInserts === "" ? 0 : 1),
							);
