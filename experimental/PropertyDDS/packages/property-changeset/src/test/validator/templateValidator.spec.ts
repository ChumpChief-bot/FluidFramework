	...
	describe("Template Validation", () => {
		...
		describe("input validation", () => {
			it("fail: empty template", () => {
				const expectations = function (result) {
		...
		describe("typeid validation", () => {
			...
			it("fail: missing semver", () => {
				const template = JSON.parse(JSON.stringify(badMissingSemverInTypeid.templateSchema));
				const expectations = function (result) {
					expect(result).property("isValid", false);
					expect(result.typeid).to.equal(template.typeid);
					expect(result.errors.length).to.be.at.least(1);
			...
			it("fail: invalid semver 1", () => {
				const template = JSON.parse(JSON.stringify(badInvalidSemverInTypeid.templateSchema));
			...
			it("fail: previous template: invalid semver", () => {
				const templatePrevious = JSON.parse(JSON.stringify(goodPointId.templateSchema));
		...
		describe("template versioning", () => {
			...
			describe("same version", () => {
				...
				it("fail: changed 'value'", () => {
					const templatePrevious = JSON.parse(JSON.stringify(goodUIBorder.templateSchema));
					const template = JSON.parse(JSON.stringify(templatePrevious));
		...
		describe("Constants", () => {
			...
			const expectationsGenerator = function (msg) {
			...
			it("should fail if map with context key type typeid is not constant", () => {
				...
				const failExpectations = function (error) {
