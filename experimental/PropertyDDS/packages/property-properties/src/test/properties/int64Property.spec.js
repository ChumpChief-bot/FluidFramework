...
let PropertyFactory;
let nodeProp;
let containedInt64Prop;
let directInt64Prop;
let Int64;
let Uint64;
let BaseProperty;
let ChangeSet;
let TestInt64ArrayTemplate;
let MSG;
...
describe("Test Int64Property", () => {
	...
	it("should correctly serialize/deserialize", () => {
		...
		let serialized = containedInt64Prop.serialize({ dirtyOnly: true });
		...
		let deserializeResult = directInt64Prop.deserialize(
		...
		const otherNodeProp = PropertyFactory.create("autodesk.tests:Int64TestID-1.0.0");
	...
	describe("squashing", () => {
		...
		const testChangeSetSquashing = function (in_options) {
			const testProperty = PropertyFactory.create("autodesk.tests:Int64TestID-1.0.0");
			const nodeTestProperty = PropertyFactory.create("NodeProperty");
			const int64InNodeProperty = PropertyFactory.create("Int64");
			nodeTestProperty.insert("intProperty", int64InNodeProperty);
			const mapTestProperty = PropertyFactory.create("map<>");
			const int64InMapProperty = PropertyFactory.create("Int64");
		...
		it("should squash two ChangeSets, one inserts the other modifies the same Int64 property", () => {
			...
			const changeSet1 = new ChangeSet(nodeTestProperty.serialize({ dirtyOnly: true }));

			nodeTestProperty.cleanDirty();
			nodeTestProperty.resolvePath("myProp").setValueHigh(100);
			const changeSet2 = new ChangeSet(nodeTestProperty.serialize({ dirtyOnly: true }));

			var nodeTestProperty = PropertyFactory.create("NodeProperty");
			var int64Property = PropertyFactory.create("Int64");
			int64Property.setValueHigh(100);
			nodeTestProperty.insert("myProp", int64Property);
			const expectedChangeSet = nodeTestProperty.serialize({ dirtyOnly: true });
	...
	describe("rebasing", () => {
		...
		const getIntPropertyFromNode = function (in_testProperty) {
			return in_testProperty._properties.myInt64;
		};

		const getIntPropertyFromMap = function (in_testProperty) {
		...
		const testRebasingInner = function (
			...
			let changeSet2 = baseProperty2.serialize({ dirtyOnly: true });
			...
			if (in_options.compareToSequential) {
				...
				let finalChangeSet = baseProperty3.serialize({ dirtyOnly: false });
