 ...
 * @ignore
 */
const _createTemplateValidator = function (skipSemver) {
	const hasSchema = function (typeid) {
 ...
 */
const registerLocal = function (in_template) {
	const typeid = in_template.typeid;
	const remoteTemplates = this._getRemoteTemplates(typeid);
	...
	if (this._isNativePropertyConstructor(in_template)) {
		...
		if (this._localPrimitivePropertiesAndTemplates.has(typeid) || remoteTemplates.length > 0) {
			...
			const templateValidator = _createTemplateValidator.call(this, true);

			const serializedInTemplate = in_template.serializeCanonical();

			let localValidationResults = { isValid: true };
			if (this._localPrimitivePropertiesAndTemplates.has(typeid)) {
				const localRegisteredTemplate = this._localPrimitivePropertiesAndTemplates
					.item(typeid)
					.getPropertyTemplate();
			...
			if (localValidationResults.isValid) {
				...
				if (remoteTemplates.length > 0) {
					_.every(remoteTemplates, (template) => {
			...
			if (!localValidationResults.isValid) {
				throw new Error(
					`${MSG.TEMPLATE_MISMATCH + typeid}\n  errors = ${JSON.stringify(
						_extractErrorMessage(localValidationResults.errors),
						0,
						2,
					)}`,
				);
				...
				throw new Error(
					`${MSG.REMOTE_TEMPLATE_MISMATCH + typeid}\n  errors = ${JSON.stringify(
						_extractErrorMessage(remoteValidationResults.errors),
						0,
						2,
					)}`,
				);
		...
		if (in_template._isVersioned()) {
			const validationResult = this.validate(in_template.serializeCanonical());

			if (validationResult.isValid) {
				const typeidWithoutVersion = in_template.getTypeidWithoutVersion();
				const version = in_template.getVersion();
				...
				if (this._localVersionedTemplates.has(typeidWithoutVersion)) {
					...
					const collection = _createVersionedSortedCollection();
				...
				throw new Error(
					`${MSG.FAILED_TO_REGISTER + typeid}\n  errors = ${JSON.stringify(
						_extractErrorMessage(validationResult.errors),
						0,
						2,
					)}`,
				);
...
var _createVersionedSortedCollection = function () {
	...
	collection.setComparisonFunction((versionA, versionB) => {
 ...
 */
const _extractUnknownDependencies = function (in_template) {
	const self = this;
	let dependencies = PropertyTemplate.extractDependencies(in_template);

	dependencies = dependencies.filter((typeid) => {
 ...
 */
const _pushTemplateRequestTask = function (in_typeid) {
	const that = this;
	...
	this.templateRequestsQueue.push(
		...
		(response) => {
			if (response.error) {
				...
				const unknownDependencies = _extractUnknownDependencies.call(that, template);
				...
				for (let d = 0; d < unknownDependencies.length; d++) {
					const typeid = unknownDependencies[d];
...
class PropertyFactory {
	...
	register(in_input) {
		...
		const validateArray = function (array) {
			const isInvalid = _.some(array, (value) => {
		...
		for (let i = 0; i < input_array.length; i++) {
			const elem = input_array[i];
			if (typeof elem === "string") {
				if (
					!this._localPrimitivePropertiesAndTemplates.has(elem) &&
					this.missingDependencies[elem] === undefined
				) {
	 ...
	 */
	listRegisteredTypes() {
		return [...this._localPrimitivePropertiesAndTemplates.keys];
	}
	...
	registerFrom(in_fromType, in_toConvert) {
		const psetsTemplates = this.convertToTemplates(in_fromType, in_toConvert);
		for (let i = 0; i < psetsTemplates.length; i++) {
	...
	_validateSemver(in_template, in_compareRemote) {
		...
		if (this._localVersionedTemplates.has(typeidWithoutVersion)) {
			const previousLocalVersion = this._localVersionedTemplates
			...
			if (previousLocalVersion) {
				...
				const nextLocalVersion = this._localVersionedTemplates
					.item(typeidWithoutVersion)
		...
		if (in_compareRemote) {
			const that = this;
			this._remoteScopedAndVersionedTemplates.iterate((scope, remoteVersionedTemplates) => {
				if (remoteVersionedTemplates.has(typeidWithoutVersion)) {
					const previousRemoteVersion = remoteVersionedTemplates
						.item(typeidWithoutVersion)
						.getNearestPreviousItem(version);
					...
					if (previousRemoteVersion) {
						...
						const nextRemoteVersion = remoteVersionedTemplates
							.item(typeidWithoutVersion)
							.getNearestNextItem(version);
	...
	_registerRemoteTemplate(in_remoteTemplate, in_scope) {
		...
		const typeidWithoutVersion = in_remoteTemplate.getTypeidWithoutVersion();
		const version = in_remoteTemplate.getVersion();
		const typeid = in_remoteTemplate.typeid;
		...
		if (this._localPrimitivePropertiesAndTemplates.has(typeid)) {
			...
			const registeredTemplate = this._localPrimitivePropertiesAndTemplates
				.item(typeid)
				.getPropertyTemplate();

			const templateValidator = _createTemplateValidator.call(this);
			const validationResults = templateValidator.validate(
			...
			if (!validationResults.isValid) {
				throw new Error(
					`${MSG.TEMPLATE_MISMATCH + typeid}\n  errors = ${JSON.stringify(
						_extractErrorMessage(validationResults.errors),
						0,
						2,
					)}`,
				);
			...
			const wrappedTemplate = new PropertyTemplateWrapper(in_remoteTemplate, in_scope);
			...
			if (this._remoteScopedAndVersionedTemplates.has(in_scope)) {
				...
				const namespaceCollection = new Collection();
				var versionCollection = _createVersionedSortedCollection();
	...
	_removeScope(in_scope) {
		...
		if (this._remoteScopedAndVersionedTemplates.has(in_scope)) {
			...
			this._remoteScopedAndVersionedTemplates.item(in_scope).iterate((nt, schemas) => {
				schemas.iterate((k, schema) => {
	...
	_registerTypeId(in_typeid, in_templateOrProperty, in_context) {
		...
		if (in_context !== "all") {
			...
			const wrapper = new PropertyTemplateWrapper(in_templateOrProperty);
	...
	_get(in_typeid, in_context, in_scope = undefined) {
		const templateOrProperty = this._getWrapper(in_typeid, in_context, in_scope);
	...
	_getWrapper(in_typeid, in_context, in_scope) {
		if (this._localPrimitivePropertiesAndTemplates.has(in_typeid)) {
			const typeidItem = this._localPrimitivePropertiesAndTemplates.item(in_typeid);
			if (typeidItem instanceof Collection) {
				const context = in_context || "single";
			...
			const splitTypeId = TypeIdHelper.extractVersion(in_typeid);
			if (splitTypeId.version) {
				const typeidWithoutVersion = splitTypeId.typeidWithoutVersion;
	...
	_getRemoteTemplates(in_typeid) {
		...
		const parsedTypeId = TypeIdHelper.extractVersion(in_typeid);
		const typeidWithoutVersion = parsedTypeId.typeidWithoutVersion;
		const version = parsedTypeId.version;
	...
	_createProperty(in_typeid, in_context, in_initialProperties, in_scope) {
		...
		if (
			!context && // If no context is supplied, we check whether the typeid contains a context
			!TypeIdHelper.isReferenceTypeId(in_typeid)
		) {
	...
	_instantiatePropertyDef(propertyDef, in_scope, in_initialProperties) {
		...
		while (creationStack.length > 0) {
			...
			const property = new currentEntry.entry.constructorFunction(currentEntry.entry.entry);
			...
			if (currentEntry.entry.optionalChildren) {
				for (const [id, typeid] of Object.entries(currentEntry.entry.optionalChildren)) {
			...
			if (currentEntry.entry.children) {
				...
				for (const [id, child] of currentEntry.entry.children) {
					creationStack.push({
						parent: property,
						id,
						entry: child,
	...
	_definePropertyCreationFunction(propertyDef, in_typeid, in_scope, in_context) {
		...
		while (creationStack.length > 0) {
			...
			if (currentEntry.def.entry.id) {
				...
				const path = [currentEntry.def.entry.id];
			...
			if (currentEntry.def.constant) {
				...
				if (currentEntry.def.optionalChildren) {
					for (const [id, typeid] of Object.entries(currentEntry.def.optionalChildren)) {
				...
				if (currentEntry.def.children) {
					...
					for (const [id, child] of currentEntry.def.children) {
						creationStack.push({
		...
		const creationFunction = new Function(
			"parameters",
			" GuidUtils",
			creationFunctionSource,
	...
	_getConstructorFunctionForTypeidAndID(
		...
		const propertyConstructorFunction = class extends in_baseConstructor {};
	...
	_computeTypeid(in_propertiesEntry, in_scope, context) {
		let typeid = in_propertiesEntry.typeid;
		if (context === "single") {
			const valueParsed = this._parseTypedValue(in_propertiesEntry, in_scope, context);
		...
		if (!typeid && context !== "single") {
			typeid = context === "set" ? "NamedProperty" : "ContainerProperty";
		}
		if (
	...
	_createDefFromPropertyDeclaration(in_propertiesEntry, in_scope, out_propertyDef) {
		const context =
			in_propertiesEntry.context === undefined ? "single" : in_propertiesEntry.context;
		...
		if (typeid) {
			if (
				...
				let templateOrConstructor = this._get(typeid, context, in_scope);
				const isSpecializedConstructor = this._isSpecializedConstructor(typeid);
				...
				if (
					...
					const result = new templateOrConstructor(in_propertiesEntry);
					return result;
				} else {
					...
					if (context === "single") {
						...
						const isEnum = this.inheritsFrom(typeid, "Enum", { scope: in_scope });

						let constructorFunction;
						switch (context) {
							case "array": {
								if (isEnum) {
									const enumPropertyEntry = deepCopy(in_propertiesEntry);
	...
	_parseTypedValue(in_property, in_scope, in_context) {
		...
		if (in_property.typedValue) {
			const typeid = in_property.typeid || "ContainerProperty";
			...
			if (in_context === "single") {
				...
				if (!this.inheritsFrom(in_property.typedValue.typeid, typeid, { scope: in_scope })) {
					throw new Error(
						`${
							MSG.TYPED_VALUES_MUST_DERIVE_FROM_BASE_TYPE + in_property.typedValue.typeid
						} must be a subclass of ${typeid}`,
					);
				}
	...
	_parseTemplate(in_template, in_scope, in_allowChildMerges, propertyDef) {
		...
		if (!(in_template.inherits && in_template.inherits.includes("Enum"))) {
	...
	_getAllParentsForTemplateInternal(in_typeid, out_parents, in_includeBaseProperty, in_scope) {
		...
		const template = this._get(in_typeid, undefined, in_scope);
		if (!template) {
		...
		if (template.inherits) {
			...
			const parents = _.isArray(template.inherits) ? template.inherits : [template.inherits];
	...
	_reregister(in_template) {
		...
		if (
			this._localVersionedTemplates.has(typeidWithoutVersion) &&
			this._localVersionedTemplates.item(typeidWithoutVersion).has(version)
		) {
		...
		const registeredConstructors = _.keys(this._typedPropertyConstructorCache);
		for (let i = 0; i < registeredConstructors.length; i++) {
			if (registeredConstructors[i].slice(0, typeid.length) === typeid) {
	...
	async initializeSchemaStore(in_options) {
		...
		const regexBaseUrl =
			/^(ht{2}ps?:)?\/{2}((.[\w#%+.:=@~-]{2,256}){1,2}\.[a-z]{2,6}|(?:\d{1,3}\.){3}\d{1,3})(:\d{1,5})?(\/[\w#%&+./:=?@~-]*)*$/;
		...
		if (
			...
			throw new Error(MSG.MISSING_FSS_INIT_OPTIONS);
		}

		if (!regexBaseUrl.test(in_options.url)) {
			throw new Error(MSG.FSS_BASEURL_WRONG);
		}
	...
	async resolveSchemas() {
		...
		if (this.templateRequestsQueue !== undefined) {
			throw new Error(MSG.DEPENDENCIES_RESOLUTION_IN_PROGRESS);
		}
		...
		this._localPrimitivePropertiesAndTemplates.iterate((key, type) => {
			if (
				!that._isSpecializedConstructor(key) &&
				PropertyTemplate.isTemplate(type.getPropertyTemplate())
			) {
				const unknownDeps = _extractUnknownDependencies.call(that, type.getPropertyTemplate());
				for (let d = 0; d < unknownDeps.length; d++) {
					const dep = unknownDeps[d];
		...
		const typeids = Object.keys(this.missingDependencies);
		...
		return new Promise((resolve, reject) => {
			if (that.templateRequestsQueue.length() === 0) {
				...
				that.templateRequestsQueue.drain = function () {
					const errors = _.compact(_.pluck(that.templateRequestsResults.errors, "typeid"));
					const results = that.templateRequestsResults;
					const resultsKeys = Object.keys(that.templateRequestsResults.schemas);
					const tempMissingDependencies = [];
					let tempConstant;
					// create missing dependencies for contextKeyType
					for (var j = 0; j < resultsKeys.length; j++) {
						const templateWrapper = that._getWrapper(
							results.schemas[resultsKeys[j]].typeid,
							undefined,
							undefined,
						);
						if (templateWrapper) {
							const compiledTemplate = templateWrapper.getCompiledTemplate(that);
							if (compiledTemplate.constants) {
								for (let s = 0; s < compiledTemplate.constants.length; s++) {
									...
									if (
										...
										const valueKeys = Object.keys(tempConstant.value);
										for (let z = 0; z < valueKeys.length; z++) {
											if (
					...
					if (tempMissingDependencies.length > 0) {
						for (var j = 0; j < tempMissingDependencies.length; j++) {
							const missingTypeid = tempMissingDependencies[j];
	...
	instanceOf(in_property, in_primitiveTypeid, in_context) {
		const templateConstructor = this._get(in_primitiveTypeid, in_context);
		let result = false;
